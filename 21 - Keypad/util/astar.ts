...
type AStarArgs<Node, Hash, Edge extends BasicEdge<Node>> = {
    start:Node,
    goal:Node,
...
export const reconstruct_path = <Node, Hash, Edge extends BasicEdge<Node>>(
    goal_node:Node,
    node_to_hash:(node:Node)=>Hash,
    came_from:Map<Hash, {node:Node, next:Edge}>
) => {
    const path:{node:Node, next?:Edge}[] = [{node:goal_node}];
    let pointer:{node:Node, next?:Edge}|undefined = path[0];
    while(true){
        const hash_pointer = node_to_hash(pointer!.node);
...
export const shortest_path = function <Node, Hash, Edge extends BasicEdge<Node>>(
    {
        start,
        goal,
        heuristic,
    ...
    const came_from:Map<Hash, {node:Node, next:Edge}> =  new Map();
    ...
    while (open_set.length>0){
        ...
        for (const edge of adjacent(current.node)){
            ...
            if(
                ...
                came_from.set(neighbor_hash, {node:current.node, next:edge});
...
export const all_shortest_paths = function <Node, Hash, Edge extends BasicEdge<Node>>(
    {
        start,
        goal,
        heuristic,
